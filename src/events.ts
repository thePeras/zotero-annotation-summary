...
export async function extractAllAnnotations(): Promise<string | null> {
  ...
  // Detect what is currently selected in Zotero
  let selectedItemID: number | null = null;
  let selectedCollectionPath: string | null = null;
  ...
  try {
    ...
    if (ZoteroPane) {
      ...
      if (selectedCollection) {
        ...
        const segs: string[] = [];
        let cursor: any = selectedCollection;
        let guard = 0;
        ...
        if (segs.length > 0) {
          selectedCollectionPath = segs.join(" / ");
        }
      }

      // Check if an item is selected (takes precedence over collection)
      const selectedItems = ZoteroPane.getSelectedItems();
      if (selectedItems && selectedItems.length === 1) {
        const item = selectedItems[0];
        // If it's a regular item (paper), use it
    ...
    Zotero.debug("Failed to detect selection: " + e);
  }
